...
import { createOceanCurrentAtlasField } from './OceanCurrentAtlasField';
import type { OceanCurrentAtlasFieldAPI } from './OceanCurrentAtlasField';

export type OceanCurrentAtlasLayerInterface = mapboxgl.CustomLayerInterface & {
  visible: boolean;
  oceanCurrentAtlasField?: OceanCurrentAtlasFieldAPI;
...
export function oceanCurrentAtlasLayer(id: string): OceanCurrentAtlasLayerInterface {
  return {
    id,
    ...
    onAdd(map, gl) {
      this.oceanCurrentAtlasField = createOceanCurrentAtlasField(map, gl as WebGL2RenderingContext);
    ...
    async setSource(manifest: OceanCurrentManifest, baseUrl: string) {
      await this.oceanCurrentAtlasField?.setSource(manifest, baseUrl);
