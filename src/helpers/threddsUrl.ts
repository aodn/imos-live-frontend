...
export const rasterUrl = (id: OverlaySource, date: Date): string => {
  return {
    ...
    [SST_ANOMALY_MOSAIC_OVERLAY_SOURCE_ID]: (date: Date) =>
      `${baseUrl(id, date)}?bbox={bbox-epsg-3857}&version=1.3.0&COLORSCALERANGE=-10,10&REQUEST=GetMap&LAYERS=sst_anom_mosaic&styles=raster/div-RdYlBu-inv&crs=EPSG:3857&format=image/png&transparent=true&width=256&height=256`,
  }[id](date);
};
...
export const rasterLegendUrl = (id: OverlaySource, date: Date): string => {
  return {
    ...
    [SST_ANOMALY_MOSAIC_OVERLAY_SOURCE_ID]: (date: Date) =>
      `${baseUrl(id, date)}?version=1.3.0&COLORSCALERANGE=-10,10&REQUEST=GetLegendGraphic&LAYERS=sst_anom_mosaic&palette=div-RdYlBu-inv&COLORBARONLY=true&VERTICAL=false&WIDTH=443&HEIGHT=12`,
  }[id](date);
};
