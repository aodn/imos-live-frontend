 ...
 *   3. Samples LOD1 (always loaded), then replaces with finer LODs as they
 *      arrive — each resident finer chunk pops in at full opacity (progressive
 *      rendering, no animated crossfade).
 ...
 *
 * Categorical mode shares the same per-chunk replace as continuous data but
 * skips the linear lookup: each fragment picks the finer LOD's exact value,
 * rounds it to its index in `flagValues` (offset from u_value_range.x), and
 * samples the colour ramp at the centre of that texel. The atlas and ramp must
 * both be uploaded with NEAREST filtering — see HeatmapAtlasField.
 */
...
void main() {
    ...
    // LOD1 (lodIdx=0) is the base sample. It is preloaded progressively, so a
    // tile may be momentarily absent — and a failed fetch leaves it absent
    // permanently (LOD1 has no scheduler to retry it). Guard against the missing
    // slot: worldToAtlasUV would otherwise index u_slots[-1] (out of bounds →
    // undefined, typically clamped to slot 0) and smear another tile's data here.
    if (physicalSlot(lonlat, 0) < 0) discard;
    ...
    // coarser sample the moment it is loaded — finer detail pops in per-chunk as
    // tiles arrive (progressive rendering) rather than waiting for the whole LOD
    // to finish loading. (Blending RGB-encoded scalars would also invent
    // intermediate categorical values, so a hard replace is correct either way.)
    for (int i = 1; i < u_lod_count; i++) {
        int physSlot = physicalSlot(lonlat, i);
        if (physSlot >= 0) {
            vec4 finerSample = texture(u_atlas, worldToAtlasUV(lonlat, i));
            if (finerSample.a >= 0.01) finalSample = finerSample;
        }
    }
